# Benchmark Prompt: music-beat-sync
...
> **Expected output:** Video with hype beat-synced background music, scene cuts aligned to beats
...
```
Run a full e2e performance benchmark of the music-beat-sync skill. I've sent you the JSON template (benchmark-run.json) and recording guide (recording-guide.md) — fill in the JSON with your actual measured values.

## What to benchmark

Add hype beat-sync music to this video:
...
1. Clean: `rm -f /tmp/bench_bs_* /tmp/beat-sync/* 2>/dev/null`
3. Record wall start: `echo "WALL_START=$(date +%s%N)"`
4. Install skill "music-beat-sync" from Pika Staging Skill Hub. Time each sub-step:
   a. GET `https://skills-cms.pika-labs.app/api/hub/install?id=music-beat-sync` → response has `bundleUrl` and `version`
   b. If `/data/.pikabot/workspace/skills/music-beat-sync/SKILL.md` exists, remove the old folder first (this is an upgrade)
   c. Download zip from `bundleUrl`, extract to `/data/.pikabot/workspace/skills/music-beat-sync/` (SKILL.md must be at root — flatten if nested in subfolder)
...
6. Run each stage individually and time it:
   - `ffprobe` (probe)
   - Scene detection: `ffmpeg -i <input> -vf "select='gt(scene,0.35)',showinfo" -f null -`
   - Full skill run: `python3 $PIKABOT_SKILLS_DIR/music-beat-sync/scripts/beat_sync.py --video <input> --vibe hype --dev`
...
1. Save the complete filled-in JSON to `/tmp/bench_bs_result.json`
...
- task: name="music-beat-sync", version from CMS
- platform: name="pikabot", runtime="EKS pod", llm_model
- phases: all timing values in seconds (convert ms to s)
- phases.execution.steps: beat_sync_full (aggregate), plus substeps: music_generation_minimax, beat_alignment_encode, audio_mix. Note: substeps are breakdowns of beat_sync_full, not additive
...
````
Run a full e2e performance benchmark of the music-beat-sync skill. First read these two files:
...
## Setup
- Skill location: ~/code/nova3-agent-skills/music-beat-sync/
...
3. Download test video:
   ```bash
   T=$(date +%s%N); curl -sL -o /tmp/bench_bs_input.mp4 "https://mellis-test-assets.s3.us-east-1.amazonaws.com/video-translation-tests/charli-daily-vlog.mp4"; echo "download_video: $(( ($(date +%s%N) - T) / 1000000 ))ms"
   ```
4. Probe video:
   ```bash
   T=$(date +%s%N); ffprobe -v quiet -print_format json -show_format -show_streams /tmp/bench_bs_input.mp4 > /tmp/bench_bs_probe.json; echo "probe_video: $(( ($(date +%s%N) - T) / 1000000 ))ms"
   ```
5. Scene detection:
   ```bash
   T=$(date +%s%N); ffmpeg -i /tmp/bench_bs_input.mp4 -vf "select='gt(scene,0.35)',showinfo" -f null - 2>/tmp/bench_bs_scenes.txt; SCENES=$(grep -c "pts_time" /tmp/bench_bs_scenes.txt || echo 0); echo "scene_detection (${SCENES} cuts): $(( ($(date +%s%N) - T) / 1000000 ))ms"
   ```
6. Full skill run:
   ```bash
   T=$(date +%s%N)
   ...
   ~/.venvs/captions/bin/python \
     ~/code/nova3-agent-skills/music-beat-sync/scripts/beat_sync.py \
     --video /tmp/bench_bs_input.mp4 --vibe hype --dev 2>/tmp/bench_bs_skill_log.txt
   echo "beat_sync_full: $(( ($(date +%s%N) - T) / 1000000 ))ms"
   echo "output_size: $(du -k /tmp/beat-sync/beat_sync_output.mp4 | cut -f1) KB"
   cat /tmp/bench_bs_skill_log.txt
   ```
7. Record wall end: `WALL_END=$(date +%s%N); echo "WALL_TOTAL: $(( (WALL_END - <WALL_START_VALUE>) / 1000000 ))ms"` — substitute the nanosecond value from step 2
...
~/code/agent-skill-running-benchmarking/runs/YYYY-MM-DD_claudecode_<model>_music-beat-sync_runN/result.json
...
````
Run a full e2e performance benchmark of the music-beat-sync skill via a Claude Managed Agent. First read these two files:
- ~/code/agent-skill-running-benchmarking/templates/benchmark-run.json (output format)
- ~/code/agent-skill-running-benchmarking/templates/recording-guide.md (how to fill fields)
...
- Agent ID: `agent_011CZuSWQpCT9CJauqGnETXu` (pika-beat-sync)
...
2. Source the API key, record wall start, and create a session:
   ...
   AGENT_ID="agent_011CZuSWQpCT9CJauqGnETXu"
   ...
   SESSION=$(curl -sS https://api.anthropic.com/v1/sessions \
     ...
     -d "{
       ...
       \"title\": \"Benchmark: music-beat-sync $(date +%Y-%m-%d_%H%M)\"
     }")
   SESSION_ID=$(echo "$SESSION" | python3 -c "import json,sys; print(json.loads(sys.stdin.read())['id'])")
   echo "Session: $SESSION_ID"
   ```

3. Send the task message. **Important:** the task message tells the agent to use `nohup` for the beat-sync script (bash tool has a 295s timeout; MiniMax music generation alone takes 60-175s, so the full pipeline will exceed this):
   ```bash
   curl -sS "https://api.anthropic.com/v1/sessions/${SESSION_ID}/events" \
     ...
     -d '{
       "events": [{
         "type": "user.message",
         "content": [{"type": "text", "text": "Add hype beat-synced background music to this video:\nhttps://mellis-test-assets.s3.us-east-1.amazonaws.com/video-translation-tests/charli-daily-vlog.mp4\n\nPrepend your ENVIRONMENT SETUP exports (PIKA_AGENT_API_KEY, PIKA_API_BASE_URL, PYTHONPATH, nova3_common symlink) to every bash command that runs Python scripts.\n\nFollow these steps exactly, timing each one:\n\n## Step 1: Download\n```bash\nT_START=$(date +%s%N)\ncurl -sL \"https://mellis-test-assets.s3.us-east-1.amazonaws.com/video-translation-tests/charli-daily-vlog.mp4\" -o /mnt/session/outputs/input.mp4\nT_END=$(date +%s%N)\necho \"download_ms: $(( (T_END - T_START) / 1000000 ))\"\nls -lh /mnt/session/outputs/input.mp4\n```\n\n## Step 2: Run beat-sync script\nIMPORTANT: The bash tool has a 295s timeout. MiniMax music generation alone takes 60-175s, so the full pipeline WILL exceed this. You MUST use nohup + background + poll pattern:\n```bash\nnohup bash -c '\n# <your ENVIRONMENT SETUP exports here>\nT_START=$(date +%s%N)\npython /workspace/skills/music-beat-sync/scripts/beat_sync.py --video /mnt/session/outputs/input.mp4 --vibe hype --dev 2>/mnt/session/outputs/ffmpeg.log\nT_END=$(date +%s%N)\necho \"beat_sync_ms: $(( (T_END - T_START) / 1000000 ))\" > /mnt/session/outputs/timing.txt\ncp /tmp/beat-sync/beat_sync_output.mp4 /mnt/session/outputs/beat_sync_output.mp4 2>/dev/null\necho \"output_size_kb: $(du -k /mnt/session/outputs/beat_sync_output.mp4 | cut -f1)\" >> /mnt/session/outputs/timing.txt\necho \"DONE\" >> /mnt/session/outputs/timing.txt\n' > /mnt/session/outputs/beat_sync_stdout.log 2>&1 &\necho \"PID: $!\"\n```\nThen poll every 15s (MiniMax generation is slow):\n```bash\nwhile ! grep -q DONE /mnt/session/outputs/timing.txt 2>/dev/null; do sleep 15; done\ncat /mnt/session/outputs/timing.txt\ncat /mnt/session/outputs/beat_sync_stdout.log\n```\nRedirect ffmpeg stderr to a file (2>/mnt/session/outputs/ffmpeg.log) — do NOT let it print to stdout, it bloats your context.\n\n## Step 3: Upload to CDN\nUse your ENVIRONMENT SETUP exports, then:\n```bash\nT_START=$(date +%s%N)\npython3 -c \"\nimport sys; sys.path.insert(0, '/workspace/skills')\nfrom nova3_common.cdn import upload_to_cdn\nfrom nova3_common.config import get_proxy_config\nbase_url, api_key = get_proxy_config()\nurl = upload_to_cdn('/mnt/session/outputs/beat_sync_output.mp4', base_url, api_key)\nprint(f'CDN_URL: {url}')\n\"\nT_END=$(date +%s%N)\necho \"upload_ms: $(( (T_END - T_START) / 1000000 ))\"\n```\n\nReport all timing values and the CDN URL."}]
       }]
     }'
   ```
...
Fill in the benchmark-run.json template and save to:
~/code/agent-skill-running-benchmarking/runs/YYYY-MM-DD_managed-claude-agent_<model>_music-beat-sync_runN/result.json
...
- phases.execution.steps: map tool calls by command content (curl=download, beat_sync.py=execution, upload_to_cdn=delivery)
...
Fill in the benchmark-run.json template and save to:
~/code/agent-skill-running-benchmarking/runs/YYYY-MM-DD_codex_<model>_music-beat-sync_runN/result.json
...
- **Clean caches before each run:** `rm -f /tmp/bench_bs_* /tmp/beat-sync/*`
- **MiniMax API latency:** Music generation takes 60-175s and varies wildly between runs. This is the dominant variable.
- **Scene detection:** Expect ~26 cuts for this video. Auto mode selects sync (re-time cuts to beats).
- **Output size:** ~16MB for 67.8s video.
- **Short-track issue:** MiniMax often generates tracks shorter than the video (15-31s for 67.8s). The skill loops the track automatically.
- **Managed agent bash timeout:** 295s per tool call. MiniMax generation alone takes 60-175s, plus scene detection + ffmpeg encoding. The full pipeline WILL exceed 295s. Always use `nohup` + background + poll pattern.
...
2. **Create the run directory and save**:
   ```bash
   RUN_DIR=~/code/agent-skill-running-benchmarking/runs/YYYY-MM-DD_pikabot_sonnet-4.6_music-beat-sync_runN
   mkdir -p "$RUN_DIR"
...
3. **Generate report.md** from the JSON:
   ...
   p = d['phases']
   tk = d['tokens']
   print(f'# music-beat-sync Benchmark — pikabot / {d[\"platform\"][\"llm_model\"]}')
   print(f'')
   print(f'**Date:** {d[\"meta\"][\"date\"]}')
   print(f'**Platform:** pikabot on {d[\"platform\"][\"runtime\"]}')
   print(f'**Skill:** music-beat-sync {d[\"task\"][\"version\"]}')
   print(f'')
   print(f'## Results')
   print(f'')
